...
final class AuditCommand {
    ...
    private static boolean view(AuditModule module,CommandSender sender,String[] args){
        ...
        String kind=alias(args[1]);if(kind.equals("bau")){viewContainer(module,player,args);return true;}
    ...
    private static UUID playerId(String raw){try{return UUID.fromString(raw);}catch(IllegalArgumentException ignored){return Bukkit.getOfflinePlayer(raw).getUniqueId();}}
