...
public enum Msg {
    ...
    BundleContentsExtracted("Removed a Bundle from ~name~'s inventory, safely extracted its contents (~item~) @"),
    BundleOverflowDropped(
            "~count~ item(s) extracted from a Bundle did not fit in the inventory of ~player~ and were dropped on the ground nearby @"),
    BundleOverflowDiscarded(
            "~count~ item(s) extracted from a Bundle did not fit in the inventory of ~player~ and were discarded to prevent duplication (DropBundleItemsOnGround is disabled) @"),
